...
namespace W31UL9_HSZF_2024252
{
    internal class Program
    {
        ...
        static bool ShowMenu(IServiceProvider serviceProvider)
        {
            ...
            Console.WriteLine("5. Exportalas csv-be");
        ...
        public static IServiceProvider RegisterServices()
        {
            IServiceProvider serviceProvider = new ServiceCollection()
                                ...
                                .AddTransient<EntityNotFoundException>()
        ...
        public static void ExportToCSV(IServiceProvider serviceProvider)
        {
            ...
            var tripService = serviceProvider.GetService<TripService>();
            var trips = tripService.GetAll().ToList();

            StreamWriter writer = new StreamWriter("cars.csv");
            writer.WriteLine("Id,Model,TotalDistance,DistanceSinceLastMaintenance");
            foreach (var car in cars)
            {
                writer.WriteLine($"{car.Id},{car.Model},{car.TotalDistance},{car.DistanceSinceLastMaintenance}");
            }

            writer = new StreamWriter("customers.csv");
            writer.WriteLine("Id,Name,Balance");
            foreach (var customer in customers)
            {
                writer.WriteLine($"{customer.Id},{customer.Name},{customer.Balance}");
            }

            writer = new StreamWriter("trips.csv");
            writer.WriteLine("Id,CarId,CustomerId,Distance,Cost");
            foreach (var trip in trips)
            {
                writer.WriteLine($"{trip.Id},{trip.CarId},{trip.CustomerId},{trip.Distance},{trip.Cost}");
            }

            Console.WriteLine("Sikeresen exportalva csv-be");
        }
